...
static void qmp_marshal_output_size(uint64_t ret_in,
                                QObject **ret_out, Error **errp)
{
    Visitor *v;

    v = qobject_output_visitor_new(ret_out);
    if (visit_type_size(v, "unused", &ret_in, errp)) {
        visit_complete(v, ret_out);
    }
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_type_size(v, "unused", &ret_in, NULL);
    visit_free(v);
}
...
void qmp_marshal_query_migrate_cache_size(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_marshal_output_size(retval, ret, errp);
